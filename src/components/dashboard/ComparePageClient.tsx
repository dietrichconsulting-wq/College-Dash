...
export function ComparePageClient({ profile }: ComparePageClientProps) {
  ...
  const METRICS: { key: keyof ComparedSchool; label: string; subtitle?: string; tooltip?: string; format: (v: unknown) => string }[] = [
    ...
    { key: 'yourChance', label: 'Your Chance', subtitle: 'based on your GPA & SAT · not a guarantee', tooltip: 'AI-estimated admission probability using your GPA and SAT score compared to the school\'s published admit rate and SAT range. Rounded to nearest 5%. This is an estimate, not a guarantee of admission.', format: v => v != null ? `~${v}%` : '—' },
